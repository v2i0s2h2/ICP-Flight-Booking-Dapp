...
const Flights = ({ fetchBalance }) => {
  ...
  const createNewFlight = async (data) => {
    ...
    let arg = {
      ...
      departureTime:BigInt(data.departureTime), 
      name:data.name, 
      pricePerPerson:BigInt(data.pricePerPerson), 
      description:data.description, 
      seats:BigInt(data.seats), 
  ...
  const update = async (data) => {
    try {
      setLoading(true);
      updateFlight(data).then((resp) => {
        getFlights();
        toast(<NotificationSuccess text="Flight Added." />);
      });
    } catch (error) {
      console.log({ error });
      toast(<NotificationError text="Failed to add flight." />);
    } finally {
      setLoading(false);
    }
  };
